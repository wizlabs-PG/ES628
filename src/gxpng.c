...
static void  ch3_clear( png_t *png, color_tt color)
//-------------------------------------------------------------------------------
{
    byte    *image_data;
    rgb_t   *ptr;                                                        // 3 ����Ʈ ũ���� �����͸� ����               
...
static void ch3_get_pixel( png_t *png, int coor_x, int coor_y, color_tt *color)
//-------------------------------------------------------------------------------
{
    byte    *image_data;
    byte    *ptr;
...
static void ch3_set_pixel( png_t *png, int coor_x, int coor_y, color_tt color)
//-------------------------------------------------------------------------------
{
    byte    *image_data;
    byte    *ptr;
...
static void  ch3_hline( png_t *png, int x_1st, int x_2nd, int coor_y, color_tt color)
//-------------------------------------------------------------------------------
// ����: ������ �׸���.
{
    byte    *image_data;
...
static void  ch3_vline( png_t *png, int coor_x, int y_1st, int y_2nd, color_tt color)
//-------------------------------------------------------------------------------
// ����: �������� �׸���.
{
    byte    *image_data;
...
static void  ch4_clear( png_t *png, color_tt color)
//-------------------------------------------------------------------------------
{                 
    byte       *image_data;
    color_tt    *ptr;
    ...
    image_data  = png->mapped;
    ptr         = (color_tt *)image_data;
    
    for ( ndx = 0; ndx < png->width*png->height; ndx++)
       *ptr++ = color;
}

static void ch4_get_pixel( png_t *png, int coor_x, int coor_y, color_tt *color)
//-------------------------------------------------------------------------------
{
    byte    *image_data;
    byte    *ptr;
...
static void ch4_set_pixel( png_t *png, int coor_x, int coor_y, color_tt color)
//-------------------------------------------------------------------------------
{
    byte    *image_data;
    byte    *ptr;
...
static void  ch4_hline( png_t *png, int x_1st, int x_2nd, int coor_y, color_tt color)
//-------------------------------------------------------------------------------
// ����: ������ �׸���.
{
    byte    *image_data;
...
static void  ch4_vline( png_t *png, int coor_x, int y_1st, int y_2nd, color_tt color)
//-------------------------------------------------------------------------------
// ����: �������� �׸���.
{
    byte    *image_data;
...
{                   
    ...
    png->font_color     = gx_color( 255, 255, 255, 255);                // �⺻ �۾� ������ ���
    png->font           = NULL;
    ...
    png->get_pixel_png  = ch4_get_pixel;
    png->set_pixel_png  = ch4_set_pixel;
    png->hline          = ch4_hline;
    png->vline          = ch4_vline;
...
// ����: ä�� 3�� 4�� �и��Ͽ� ���� �Լ� ����
{
    static png_t  *png;                                                 // static : might be clobbered by 'longjmp' or 'vfork' warinig �޽����� ���ֱ� ����
    FILE          *fp;
...
// original
/*
    if ( NULL == ( fp = fopen( filename, "rb")) )                       // ������ ���ų� ���⿡ �����ߴٸ�
    {
...
*/
    ...
    if  (NULL == (fp))
	{
		...
		if(file_len>4)
		{
			memcpy(&filename[file_len-4], EXT_BIG_PNG, 4);
			fp = fopen( filename, "r+b");	// .PNG
			if  ( NULL == ( fp) )
			{
				printf( "gx_png_open(): no file.\n");
				free_png_resource( png, fp);
				return NULL;
			}
		}
...
//  }
    ...
    if ( image_channels == 3)
    {
       ...
       png->get_pixel_png   = ch3_get_pixel;
       png->set_pixel_png   = ch3_set_pixel;
       png->hline           = ch3_hline;
       png->vline           = ch3_vline;
